...
import {
  autoImportCardFieldsForCard,
  autoImportCardFieldsFromPageDataSource,
...
import type { CardArrayFilterCondition } from '@mvp-vue/schema'
...
import CardListEmptyDisplaySection from '../card-list/CardListEmptyDisplaySection.vue'
...
async function runCardFieldImport() {
  ...
  cardFieldImportErr.value = ''
  try {
    const dataPath = (props.comp.props.dataPath as string) ?? 'data'
    const arrayFilters = (props.comp.props.arrayFilters as CardArrayFilterCondition[]) ?? []
    const compWl = parseImportFieldWhitelist(props.comp.props.importFieldWhitelist)
    let fields: import('@mvp-vue/schema').CardFieldConfig[]
    if (usesPageDataSource.value && props.comp.dataSourceId) {
      ...
      fields = await autoImportCardFieldsFromPageDataSource(
        ds,
        dataPath,
        arrayFilters,
      ...
      fields = await autoImportCardFieldsForCard({
...
<template>
  <div class="flex flex-col gap-2.5">
    ...
    <p v-if="usesPageDataSource" :class="[PROP_HINT, 'text-amber-200/90']">
      已绑定页面数据源，下方 API URL 在渲染时不生效。
    </p>

    <!-- API URL -->
    <label class="block" :class="usesPageDataSource ? 'opacity-50 pointer-events-none' : ''">
    ...
    <!-- Fixed params -->
    <div class="border-t border-white/10 pt-2" :class="usesPageDataSource ? 'opacity-50 pointer-events-none' : ''">
    ...
    <div class="border-t border-white/10 pt-2" :class="usesPageDataSource ? 'opacity-50 pointer-events-none' : ''" />
    <label class="block" :class="usesPageDataSource ? 'opacity-50 pointer-events-none' : ''">
      <span :class="PROP_LABEL">数据刷新间隔（秒，0=不刷新）</span>
      <EditorNumberInput
        :min="0"
        :max="3600"
    ...
    <label class="block">
      <span :class="PROP_LABEL">内边距 (itemInset)</span>
      <EditorNumberInput
        :min="0"
        :max="40"
        :class="PROP_NUMBER_WRAP"
  ...
  <ComponentPropsJsonDialog
    ...
    :title="propsJsonDialog === 'import' ? '导入 card 属性' : '导出 card 属性'"
    :props="comp.props"
