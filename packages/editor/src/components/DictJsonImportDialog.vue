...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click="onClose">
    <div class="bg-[#1a2332] border border-white/20 rounded-lg shadow-2xl p-5 w-[500px]" @click.stop>
      ...
      <textarea
        v-model="text"
        class="w-full h-60 bg-white/5 border border-white/10 rounded-md px-3 py-2 text-xs text-slate-200 font-mono resize-none outline-none focus:border-indigo-400/50 transition-colors"
        placeholder="粘贴 JSON..."
      />
